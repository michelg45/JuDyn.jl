"""
    create_h5_file_static

        Function creating the .h5 file for archiving the results of a static analysis
        ...
        Calling sequence:
            ...
            create_h5_file_static(h5_file,Npas,eigvals,eig_freq,max_vals)

"""
function create_h5_file_static(h5_file::String, Npas::Int,eigvals::Bool,eig_freq::Int,max_vals::Int)
...
Nbounds > 0 && (g14 = create_group(file,"multipliers"))
Nbounds > 0 && (g15 = create_group(file,"bounds"))

# N_visco_beams | N_visco_shells > 0 && (g3 = create_group(file,"ydot"))

dsets[1] = create_dataset(g1,"times",datatype(Float64), dataspace(Npas,1))
dsets[2] = create_dataset(g2,"y",datatype(Float64), dataspace(Npas,Ndofs))
dsets[3] = create_dataset(g3,"ydot",datatype(Float64), dataspace(Npas,Ndofs))
dsets[4] = create_dataset(g4,"nitmax",datatype(Int), dataspace(Npas,1))
dsets[5] = create_dataset(g5,"p",datatype(Float64), dataspace(Npas,Ndofs))
dsets[7] = create_dataset(g7,"pot_energy",datatype(Float64), dataspace(Npas,1))
dsets[8] = create_dataset(g8,"str_energy",datatype(Float64), dataspace(Npas,1))
dsets[9] = create_dataset(g9,"ext_work",datatype(Float64), dataspace(Npas,1))
dsets[13] = create_dataset(g13,"node_positions",datatype(Float64), dataspace(Npas,3,Nnodes))
dsets[30] = create_dataset(g30,"model_data",datatype(Int), dataspace(20,1))

Nbounds > 0 && (dsets[14] = create_dataset(g14,"multipliers",datatype(Float64), dataspace(Npas,Nbounds)))
Nbounds > 0 && (dsets[15] = create_dataset(g15,"bounds",datatype(Float64), dataspace(Npas,Nbounds)))
# N_visco_beams | N_visco_shells > 0 && (dsets[3] = create_dataset(g3,"ydot",datatype(Float64), dataspace(Npas,Ndofs)))
...
dsets[16] =create_dataset(gshells,"stresses",datatype(Float64), dataspace(Npas,12,N_shells));
dsets[17] =create_dataset(gshells,"numbers", datatype(Int), dataspace(N_shells,1));
dsets[24] =create_dataset(gshells,"strains",datatype(Float64), dataspace(Npas,12,N_shells)))
N_beams > 0 && (gbeams = create_group(file,"beams"); 
dsets[18] =create_dataset(gbeams,"stresses",datatype(Float64), dataspace(Npas,6,N_beams));
dsets[19] =create_dataset(gbeams,"numbers", datatype(Int), dataspace(N_beams,1));
dsets[25] =create_dataset(gbeams,"strains",datatype(Float64), dataspace(Npas,6,N_beams)))
