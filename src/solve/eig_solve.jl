...
function eig_solve(y::Vector{Float64},Dy::Vector{Float64},ydot::Vector{Float64},max_vals::Int,itime::Int,h::Float64,niter::Int)
    ...
    for iel = 1:Nel
        ...
        if el_type == "beam"
            inv_loc, S_el = beam(nbr,y_n,Dy,ydot_np1,res,p,alpha_stiff,theta_p,matrix,itime,h,niter)
        ...
        elseif el_type == "super_beam"
            inv_loc, S_el = super_beam(nbr,y_n,Dy,ydot_np1,res,p,alpha_stiff,theta_p,matrix,itime,h)
            A[inv_loc,inv_loc] += S_el
