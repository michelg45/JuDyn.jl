...
```math
\\mathbf{b} = \\mathbf{T}(\\mathbf{\\psi})\\mathbf{a}
```
...
````{verbatim}
    julia> a = Vec3(1.0,0.0,0.0)
    Vec3([1.000000e+00, 0.000000e+00, 0.000000e+00])
    julia> psi = RV3(0.0,2.1,0.0)
...
Function computing the tangent operator `T::Mat3` from the Cartesian rotation vector `psi::RV3` 
...
function tang(psi::RV3)
    ...
    PSI=sqrt(PSI2)
    PSI > 1.0e-2 ? f_1 = (cos(PSI)-1.0)/PSI2 : f_1 = -0.5 + PSI2/24.0 - PSI2^2/720.0
    PSI > 1.0e-4 ? f_2 = (1.0-sin(PSI)/PSI)/PSI2 : f_2 = 1.0/6.0 - PSI2/120.0 + PSI2^2/5040.0
